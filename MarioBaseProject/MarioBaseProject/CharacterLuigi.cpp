...
CharacterLuigi::CharacterLuigi(SDL_Renderer* renderer, std::string imagePath, int spriteSheetWidth, int spriteSheetHeight, float keyframeSpeed, Vector2D start_position, LevelMap* map) : Character(renderer, imagePath, spriteSheetWidth, spriteSheetHeight, keyframeSpeed, start_position, map)
{
    m_lives = 3;

    m_lives_texture = new Texture2D(m_renderer);
    if (!m_lives_texture->LoadFromFile("Images/LuigiLives.png"))
    {
